        ...
        for line in f:
            if line:
                ...
                if json_content["categories"] != None:
                    categories = [val.lower().strip()
                                  for val in json_content["categories"].split(",")]
                    if "restaurants" in categories:
                        restaurant_business_ids.add(
                            json_content["business_id"])

    logging.info(
        f"Finished reading the business.json file. There are {len(restaurant_business_ids)} unique restaurants in the dataset.")

    grouped_reviews_filepath = os.path.join(
        args.yelp_data_dir, "grouped_reviews.jsonlist.gz")
    if not os.path.exists(grouped_reviews_filepath):
        logging.info(
            f'Grouped reviews not exist. Building grouped reviews at {grouped_reviews_filepath}')
        convert_data(os.path.join(args.yelp_data_dir,
                     "review.json"), grouped_reviews_filepath)
        logging.info(f'Grouped reviews file built')

    logging.info(
        f"converting grouped reviews into resutaurant only reviews and compute average of first {args.num_review} reviews")
    out_file = os.path.join(
        args.output_dir, f"yelp_10reviews_{args.num_review}avg.jsonl.gz")
    with gzip.open(grouped_reviews_filepath, 'rt') as f_in,  gzip.open(out_file, "wt") as fout:
        for l in f_in:
            ...
            if len(r['reviews']) >= args.num_review:
                ...
                tmp["avg_score"] = sum(
                    [r["reviews"][j][3] for j in range(args.num_review)])/args.num_review
                fout.write("%s\n" % json.dumps(tmp))

    if args.data_split:

        split_datapath = os.path.join(
            args.output_dir, f"{args.num_review}reviews")
        if not os.path.exists(split_datapath):
            logging.info(
                f'Split datapath not exist. Create split data dir at {split_datapath}')
        ...
        for split, ids in split_ids.items():
            ...
            logging.info(
                f"Data split length: {split} ({len(split_reviews)}), stored at: {storepath}")
